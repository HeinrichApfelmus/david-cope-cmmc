...
;;;;  the QT midi functions (uppercase names) were created by Alexander Repenning (alex@agentsheets.com)
;;;;  the other functions (lowercase names) were created by David Cope (howell@cats.ucsc.edu)
;;;;  load the cosine file first and then this file and then
;;;;  (play-events (make-events (cosine 20 24 108)))
;;;;  these have the settings of (setq *tempo* 60) 
;;;;  for different results try
;;;;  (setq *tempo* 390)
;;;;  (play-events (make-events (cosine 80 24 108)))
;;;;  for more interesting results yet (but still not creative) add a t into make-events arg list as in
;;;;  (play-events (make-events (cosine 80 24 108) t))
...
(defun SORTCAR (function lists)
  "Sorts the cars of its arg according to function."
...
(defun MY-LAST (list)
  "Returns the unlisted last element of arg."
  (first (last list)))
