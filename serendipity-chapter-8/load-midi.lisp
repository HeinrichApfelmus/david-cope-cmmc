...
;;;;;
#|(translate-midi-file)
     ((4000 27 600 3 80) (4000 39 600 3 80)
      (5000 27 600 3 80) (5000 39 600 3 80)
      (6000 27 600 3 80) (6000 39 600 3 80)...|#
;;;;;

(defun TRANSLATE-MIDI-FILE ()
  "Toplevel load function that returns events from type 1 midi files."
  (setq *data* nil)
...
;;;;;
#|(pair-lists '((1 2) (a b)(3 4)(5 6)(7 8)))
        ((1 a 3 5 7) (2 b 4 6 8))|#
;;;;;

(defun PAIR-LISTS (lists)
  "Lists first elements, then second elements."
...
;;;;;
#|(get-multi-byte-value
           4
           #<A Mac Zone Pointer Size 13600 #x44C8E530>
           4
           #<A Mac Zone Pointer Size 122 #x44C91A60>) 
(8 . 6)|#
;;;;;
...
;;;;;
#|(mff-cycling-get-byte
           #<A Mac Zone Pointer Size 13600 #x44C8E530>
           0
           #<A Mac Zone Pointer Size 122 #x44C91A60>) 
 mff-cycling-get-byte returned 2 values :
      1
      77|#
;;;;;
...
;;;;;
#|(add-duration 60 1000
                '(#<*midi-event* #x115FEC2E> #<*midi-event* #x11602B4E>))
(#<*midi-event* #x11602B4E>)|#
;;;;;
...
;;;;;
#|(interpret-data track-name 0 255 '(84 101 109 112 111)) 
  "Tempo"|#
;;;;;
...
#|(get-event-type-and-data
            ...
            24
            #<A Mac Zone Pointer Size 122 #x44C91A60>)
(31 track-name (84 101 109 112 111))|#
;;;;;
...
;;;;;
#|(make-unique-gesture-name :name gesture) 
  gesture|#
;;;;;
...
;;;;;
#|(mff-read-first-track
             #<A Mac Zone Pointer Size 13600 #x44C91A60>
             14
             #<A Mac Zone Pointer Size 122 #x44C94F90>)
mff-read-first-track returned 2 values :
        50
        ((500000 . 0))|#
;;;;;
...
#|(read-header-chunk
             #<A Mac Zone Pointer Size 13600 #x44C91A60>
             #<A Mac Zone Pointer Size 122 #x44C94F90>) 
   read-header-chunk returned 4 values :
        14
        1
        2
        500|#
...
;;;;;
#|(mff-read #1P"Macintosh HD:Applications (Mac OS 9):mcl:MCL 4.3.1:untitled")
#<*gesture* #x11EBCC16>|#
;;;;;
...
;;;;;
#|(load-from-midi-menu-item-action) 
 #<*gesture* #x123F3D16>|#
;;;;;
...
;;;;;
#|(get-number-of-bytes
             '(#<*midi-event* #x12CFF1C6> #<*midi-event* #x12CFF1F6>
               #<*midi-event* #x12CFF226> #<*midi-event* #x12CFF256>))
18|#
;;;;;
...
#|(write-time-tag
             0
             #<A Mac Zone Pointer Size 13600 #x44CBE140>
             58
             #<A Mac Zone Pointer Size 122 #x44C98850>)
59|#
;;;;;
...
#|(parse-events-in-main
             ...
             :end
             nil) 
   (#<*midi-event* #x12CFF1C6>
    #<*midi-event* #x12CFF1F6>
    #<*midi-event* #x12CFF226>
    #<*midi-event* #x12CFF256>)|#
;;;;;
...
(defun GET-FILENAME (pathname)
  "Returns the filename from the pathname."
  (read-from-string 
   (mac-namestring 
...
(defun TRIM-REPEATING-NOTES (events)
  (if (null (rest events)) events
      (cons (trim (first events)(rest events))
            (trim-repeating-notes (rest events)))))

;;;;;
#|(trim '(0 60 1000 1 127) '((1000 60 1000 1 127)))
(0 60 970 1 127)|#
;;;;;

(defun TRIM (event events)
  (cond ((null events) event)
        ((and (equal (first (first events))(+ (first event)(third event)))
              (equal (second event)(second (first events))))
         (append (firstn 2 event)
                 (list (- (third event) (* (/ *tempo* 60) 30)))
                 (nthcdr 3 event)))
        ((> (first (first events))(+ (first event)(third event))) event)
        (t (trim event (rest events)))))

;;;;;
#|(create-note-offs '((1000 60 1000 1 127)))
((1000 60 1000 1 127) (2000 60 0 1 0))|#
;;;;;

(defun CREATE-NOTE-OFFS (events)
  "Creates the note off events."
  (if (null events) nil
      (append (if (find-new-note (first events)(rest events))
                (list (first events))(list (first events)
                                           (append (list (+ (first (first events))(third (first events))))
                                                   (list (second (first events)))
                                                   '(0)
                                                   (list (fourth (first events)))
                                                   (list 0))))
              (create-note-offs (rest events)))))


;;;  you'll want to check to see if not having a note off in channels 
;;;  where a new note occurs immediately after in the same channel before
;;;  creating a new note off

;;;;;
#|(find-new-note '(0 60 1000 1 127) '((1000 60 1000 1 127)))
T|#
;;;;;

(defun FIND-NEW-NOTE (event events)
  "True if the event fits before the first of events."
  (cond ((null events)())
        ((events-match event events)
         t)
        (t ())))

;;;;;
#|(events-match '(0 60 1000 1 127) '((0 60 1000 1 127)))
NIL|#
;;;;;

(defun EVENTS-MATCH (event events)
  "Only true if the event fits before the first of events."
  (cond ((null events)())
        ((> (very-first events)(+ (first event)(third event))) ())
        ((= (+ (first event)(third event))(very-first events)) t)
        (t (events-match event (rest events)))))
...
(defun MAKE-A-MIDI-GESTURE nil
  ...
  (let* ((midi-work (setq *algorithmic-composition* (mapcar #'(lambda (x)
                                                            ...
                                                            (sortcar '< (trim-repeating-notes *events*)
